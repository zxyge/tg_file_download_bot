...
async def stream_handler(request: web.Request):
    try:
        ...
        logging.critical(f'stream_handler: {e.with_traceback(None)}')
...
async def media_streamer(request: web.Request, message_id: int, secure_hash: str):
    ...
    if Var.MULTI_CLIENT:
        logging.info(f"Client {index} is now serving {request.url}")
    ...
    if utils.get_hash(file_id.unique_id, Var.HASH_LENGTH) != secure_hash:
        logging.debug(f"Streamer: Invalid hash for message with ID {message_id}")
        raise InvalidHash
